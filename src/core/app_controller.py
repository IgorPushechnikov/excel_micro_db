...
class AppController:
    ...
    def update_sheet_cell_in_project(self, sheet_name: str, row_index: int, column_name: str, new_value: str) -> bool:
        ...
        try:
            ...
            with ProjectDBStorage(str(db_path)) as storage:
                # 1. Найти sheet_id по имени листа
                ...
                # 2. Получить старое значение для истории (опционально, можно передавать из SheetEditor)
                # Для простоты, запросим его из БД
                ...
                if update_success:
                    ...
                    history_success = storage.save_edit_history_record(
                        ...
                        old_value=old_value,
                        new_value=new_value,
                        # user="GUI_User" # Можно добавить, если будет система пользователей
                    )
                    if history_success:
